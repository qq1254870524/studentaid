...
APP_TITLE = "StudentAid 批量处理工具 - 第八步实测"
...
def step_4_judge_password_recovery(
    ...
    if "An unknown error has occurred" in body_text:
        raise RuntimeError("StudentAid 返回未知错误，请稍后重试或重新打开浏览器")
    ...
    if "Recover my account with a photo ID" in body_text:
        return "can_recover"
    if not page.locator(FIRST_NAME_SELECTOR).is_visible():
        return "can_recover"
    return "unknown"
