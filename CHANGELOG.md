# v0.32.0 HTTP 快速版源码发布 - 2026-08-19

- 汇总发布 `ait23.py` 至 `ait32.py` 已验证更新，最新正式入口保持为 `ait32.py`。
- GitHub Release 定位为 HTTP 快速版源码；GUI 选择 `http` 即使用独立 Session、Cookie/CSRF 初始化和自动重试的官方接口直连流程。
- 更新 README 中遗留的 `ait22.py` 启动说明，补充 `requests` 依赖检查、HTTP 后端选择和当前启动文件名。
- 恢复并更新 `StudentAid.spec` 到 `ait32.py`，源码包同时提供运行与可选 PyInstaller 构建文件。
- 自动测试：`Ran 75 tests`，`OK (skipped=3)`；其余 72 项通过。
- 发布资产：最新源码 ZIP、完整更新日志和 SHA256 校验文件。

# v0.32.0 - 2026-08-18

- 修复含字段内逗号的 Tab 分隔 TXT 被误判成逗号分隔、12 列资料只读成 2 列的问题。
- 无表头身份列推断优先选择连续姓名列并排除州缩写，避免城市/州抢占 Last Name。
- 现场 `导入资料/8.18 强.txt` 4 行全部成功：每行 12 列，First/Last/Address/SSN/DOB 映射为第 3/4/7/11/12 列。
- 新增导入、原始列保留和 TXT 完整行删除回归；正式源码升级为 `ait32.py`，`ait31.py` 保持不改。

# v0.31.0 - 2026-08-18

- 新增 Windows 输出目录 ACL 自动修复：为当前账户设置 `(OI)(CI)F`，让后续 CSV 和 SQLite sidecar 自动继承可写权限。
- 批次启动和 GUI 后台清空数据库前同步修复已有累计 CSV、数据库、WAL/SHM 权限。
- 修复 `attempt to write a readonly database` 与累计结果 CSV 只能只读的问题。
- 正式源码升级为 `ait31.py`，`ait30.py` 保持不改。

﻿# 更新日志

## v0.30.0-step35-result-last-column — 2026-08-17

### 累计结果状态统一最后一列

- 正式源码新增 `ait30.py`，`ait29.py` 保持不改。
- 累计 CSV 根据全文件最大原始输入宽度补齐空单元格，Result Heading 固定为唯一最后一列；Masked Phone、Masked Email、Recovery Method 固定在其前面。
- 启动时自动识别并原子迁移旧版 `Heading / Phone / Email / Method` 行；更宽新输入首次出现时自动扩展已有累计行。
- 去重读取会移除程序补齐的尾部空格后再生成完整行键，保持输入同步和删除逻辑一致。

### 现场文件修复

- `DL 新 StudentAid累计结果.csv` 原有 43,140 行，行宽分布为 10 列 25 行、24 列 10,957 行、25 列 25,956 行、26 列 6,202 行。
- Result Heading 原来分别位于第 7/21/22/23 列；迁移后全部 43,140 行统一为 26 列，Result Heading 全部位于第 26 列。
- 迁移前后 43,140 个完整原始输入行键完全一致；Phone/Email/Recovery Method/Heading 四项结果逐行重排核对通过。

### 验证

- `ait30.py` 共执行 69 项测试：68 项通过，1 项历史 `8.11` 现场路径不存在而跳过。
- 新增混合宽度旧累计迁移、固定最后列、宽输入自动扩展、迁移幂等和完整行键保持测试。



## v0.29.0-step34-large-xlsx-persistence — 2026-08-17

### 大批量 XLSX 持久化不卡队列

- 正式源码新增 `ait29.py`，`ait28.py` 保持不改。
- 修复大型 XLSX 每 20 个明确结果触发一次整表删行重写，占住唯一结果持久化线程，导致 30 个 HTTP worker 无法获得累计 CSV 写入响应并在 300 秒后集中报错的问题。
- XLSX 运行期间只实时执行 SQLite 和累计 CSV 落盘，输入删除键在内存聚合；worker 全部结束后一次性同步输入文件，CSV/TXT 原有定时批处理不变。
- 启动恢复对 XLSX 改为只读导入后按完整行键过滤累计输出已有资料，不在启动阶段先重写整表；过滤键自动加入批末删除集合。
- XLSX 物理删行改为 ZIP worksheet XML 单遍过滤和行号重排，不再逐个调用 openpyxl `delete_rows`；样式及非目标工作表部件保持原样。
- 已停止现场卡住的旧 `ait27.py` 进程；累计 CSV 已安全保存的 20 条结果和原始输入 XLSX 均保留。

### 现场验证

- `DL新.xlsx`：43,140 条记录，现场累计 CSV 中 20 个完整行键。
- 在隔离副本上精确删除 20 条后剩余 43,120 条；累计结果键残留 0、意外新增/删除 0、差异键与累计输出完全一致。
- 新实现完成扫描、XML 重写和 XLSX 可读性验证约 26 秒；旧实现运行超过 300 秒仍未完成。
- `ait29.py` 共执行 67 项测试：66 项通过，1 项历史 `8.11` 现场文件当前不存在而跳过。

## v0.28.0-step33-gui-async-clear-database — 2026-08-17

### GUI 异步清空数据库

- 正式源码新增 `ait28.py`，`ait27.py` 保持不改。
- 修复“清空数据库”在 Tk 主线程同步执行 DELETE、WAL checkpoint 和 VACUUM，数据库较大或等待锁时导致 GUI 未响应的问题。
- 所有 SQLite 清理工作移入独立后台线程；GUI 通过已有事件队列接收成功或失败结果，窗口清理期间仍能正常刷新。
- 清理期间统一禁用开始、输入输出路径、后端、线程数及清空按钮，完成或失败后自动恢复，避免数据库维护与新批次并发。
- 清理时关闭窗口可选择等待后台事务结束后自动退出；后台线程不直接调用任何 Tk API。
- 清理对象和保护边界不变：保留数据库结构及其他表，不触碰累计 CSV、输入资料和 GUI 配置。

### 验证

- `ait28.py` 共执行 63 项测试：62 项通过，1 项历史 `8.11` 现场文件当前不存在而跳过。
- 新增慢速清理模拟，确认按钮回调在 1 秒内返回、后台线程仍在工作、GUI 控件被正确保护，完成事件处理后恢复正常。

## v0.27.0-step32-gui-clear-database — 2026-08-17

### GUI 清空数据库

- 正式源码新增 `ait27.py`，`ait26.py` 保持不改。
- GUI 按钮栏新增“清空数据库”，根据当前累计输出路径定位同目录 `studentaid.sqlite3`。
- 操作前显示绝对路径和影响范围并要求确认；确认后清空 batches/records、重置自增序列、截断 WAL 并执行 VACUUM，表结构保留。
- 累计 CSV、输入文件、GUI 配置和数据库中的其他非批次表保持不变。
- 批次运行时按钮禁用，方法入口再次检查运行状态；数据库不存在或清理异常均有明确提示。

### 验证

- `ait27.py` 共执行 62 项测试：61 项通过，1 项历史 `8.11` 现场文件当前不存在而跳过。
- 新增测试覆盖数据库清空计数、结构保留、自增序列重置、累计 CSV 不变、数据库不存在、GUI 确认流程和运行状态双重保护。

## v0.26.0-step31-full-row-dedupe-all-paths — 2026-08-17

### 完整行去重全路径统一

- 正式源码新增 `ait26.py`，`ait25.py` 保持不改。
- 去重键由原始输入行的全部字段逐格序列化生成；页面结果四列不参与输入判重，SSN 或第一列不再单独决定重复关系。
- 批次唯一任务、SQLite 完成/重试/失败分组、累计输出判重、启动同步和结果完成删行全部使用同一完整行键。
- 修复 Step30 无表头宽表在输入删行路径没有复用自动列推断的问题；CSV/XLSX 删除路径现在与导入路径共享显式表头/无表头映射解析。
- 完全相同整行仍只搜索一次并删除全部副本；同一 SSN 但任意其他列不同的资料保持独立任务。

### 8.17 工作簿验证

- 936 条有效资料产生 936 个唯一完整行键；按 SSN 统计只有 928 个唯一值。
- 其中 8 条是同 SSN 不同整行，完整行规则将其全部分别搜索、分别输出，不合并为 928 条。
- 合成无表头 XLSX 验证两个完全相同行被精确删除，同 SSN 但邮箱不同的第三行完整保留。

### 验证

- `ait26.py` 共执行 58 项测试：57 项通过，1 项历史 `8.11` 现场文件不存在而跳过；本次完整行去重和 `8.17` 现场测试全部通过。

## v0.25.0-step30-headerless-column-inference — 2026-08-17

### 无表头自动列推断

- 正式源码新增 `ait25.py`，`ait24.py` 保持不改。
- 首行无法识别为表头时，抽样最多 200 条资料统计每列的 SSN、DOB、姓名和地址特征；达到置信度后生成动态列映射，首行仍作为数据处理。
- 新增无表头宽表识别，支持 SSN、First、Middle、Last、DOB、Email、Phone、Address 及其他附加列；Middle Name 不会被误作 Last Name。
- 旧 `SSN,DOB,First,Last`、`SSN,First,Last,DOB` 和拆分月/日/年格式继续走原解析器，避免改变历史行为。
- 原始行列保持 Step27 的逐格透传，HTTP/浏览器、默认后端、去重、SQLite、累计输出和输入删行逻辑不变。

### 8.17 工作簿验证

- `导入资料\8.17 测试.xlsx`：无表头、936 条、每条 10 列。
- 自动映射 `SSN=0、First Name=1、Last Name=3、DOB=4、Address=7`，936/936 条均通过必填字段、SSN 和 DOB 校验。
- 首行未被当作表头跳过；源行范围为 1–936，输出前 10 列与输入逐格一致，追加结果后为 14 列。
- 使用首条有效资料完成一次不落盘 HTTP 搜索探针，取得明确 `Account Not Found: Create a New Account`；输入文件 SHA256 前后一致。

### 验证

- `ait25.py` 共执行 56 项测试：55 项通过，1 项因历史 `8.11` 现场文件当前不存在而按规则跳过；本次 `8.17` 现场测试通过。

## v0.24.0-step29-universal-input-adapter — 2026-08-17

### 万能表头自动适配

- 正式源码新增 `ait24.py`，`ait23.py` 保持不改。
- 输入表头由固定别名扩展为“精确别名 + 语义推断”：自动识别任意列顺序、`result_*`、`matched/found/verified` 供应方前后缀，以及中英文 SSN、DOB、姓名和地址字段。
- 同时存在原始身份与匹配结果时优先使用结果身份；结果姓名单项为空时回退原始姓名，避免因为部分结果字段为空丢弃完整资料。
- DOB 新增紧凑八位日期、ISO 时间、点分隔和中文日期支持；缺失月日的部分日期不会被编造为完整 DOB。
- 所有输入原始列仍逐格原样透传，HTTP/浏览器搜索、默认后端、整行去重、SQLite、累计输出和输入删行规则保持不变。

### 8.11 工作簿验证

- `8.11 背调手机号 SSN.xlsx`：1 个工作表、736 条数据、37 列，表头正确跳过，不再被当成资料行。
- 自动映射 `result_ssn`、`result_birth_date`、`result_first_name`、`result_last_name`、`result_address`，并保留原始姓名/地址作为回退。
- 465 条资料具备官网搜索所需的有效 SSN、完整 DOB 和姓名；271 条为缺少 SSN、缺少 DOB 或日期只有年份/零月零日等资料不完整情况。
- 有效记录输出前 37 列与输入逐格一致，末尾追加 4 个既有结果字段，总宽度 41 列。

### 验证

- `ait23.py` 基线：46/46 项测试通过。
- `ait24.py`：52/52 项测试通过；新增结果字段优先、姓名回退、未知供应方语义表头、中文表头、多日期格式和现场工作簿验证。

## v0.23.0-step28-http-search — 2026-08-17

### HTTP 搜索后端

- 正式源码新增 `ait23.py`，`ait22.py` 保持不改。
- GUI 搜索后端新增 `http`，原有 `browser-use`、`playwright` 和默认 `browser-use` 保持不变。
- HTTP worker 独立初始化官网会话和 CSRF token，复用当前会话逐条调用官方账户找回 JSON 接口；会话失效自动重新初始化一次。
- 当前官网错误码映射到既有明确结果；恢复选项中的联系方式在落盘前按官网形式遮挡，身份资料和联系方式明文不进入日志。
- HTTP 429、5xx、无效 JSON 和未知错误码继续使用既有三轮队尾重试，不把不明确响应写成完成。
- 未引入 USFull 结果表字段映射；输入解析、原始列透传、整行去重、浏览器流程、SQLite、累计输出和输入删行均保持原逻辑。

### 验证

- `ait23.py`：46/46 项测试通过，其中既有回归 40 项、HTTP 新增测试 6 项。
- 官方 HTTP 现场探针完成页面、会话、CSRF 和查询全链路，保留测试资料得到明确 `invalid_ssn` 结果。
- `8.11 背调手机号 SSN.xlsx` 只读检查：当前是 USFull 结果表结构，现有 StudentAid 输入映射识别为 0 条有效搜索资料；按本次范围未修改映射，也未改写该文件。
...
- 正式源码新增 `ait21.py`，`ait20.py` 保持不改。
- 批次队列、累计输出和启动同步统一按完整输入资料行去重，不再只比较第一列或 SSN。
- 同一 SSN 但 DOB、姓名、地址、电话、邮箱或任意原始/顺延字段不同的记录均分别进入浏览器、分别取得结果并分别输出；MyLife 的全部 20 个原始字段都参与整行比较，大小写或空白不同也不会被折叠。
- 完全相同的重复行仍只执行一次，并从输入文件删除额外副本；结果完成后只删除匹配的完整行，不影响同 SSN 的其他资料。
- SQLite 新增 `input_row_key` 并自动迁移旧数据库；完成、队尾重试、失败、停止按完整行键联动完全相同的副本。
- 页面结果四列不参与输入资料行键，累计输出中输入资料部分相同即视为同一整行。
- MyLife 新格式、旧输入格式、Continue、Cancel、浏览器复用、状态识别、持久化线程和 GUI 实时效率看板保持不变。

### 验证

- `ait20.py` 可执行基线中的34项通用回归通过；仅现场文件固定94/2断言因上一批已跑完、输入只剩表头而不再适用。
- `ait21.py`：40/40 测试通过；新增同SSN不同整行独立排队、独立输出、精确启动同步、完全重复删行，以及 MyLife 最后一列变化不得误去重测试。
...
- 正式源码新增 `ait19.py`，`ait18.py` 保持不改。
- 进度区新增完成百分比、已处理/总数、剩余、待处理、处理中、明确结果、失败和停止数量。
- 新增最近一分钟滚动处理数量、批次平均处理速度、预计剩余时间和已运行时间；进度变化时立即刷新，等待结果期间每秒刷新。
- 速度以批次首个数据库快照为基线，导入阶段已经失败的格式错误不计入浏览器吞吐量；没有有效速度样本时 ETA 显示 `--`。
- 并发 worker 旧快照晚到时不会让百分比或已处理数量倒退；批次结束后最终耗时保持冻结。
- 网页结果识别、Continue、Cancel、浏览器复用、输入删行、累计 CSV 和 Step23 持久化路径均未调整。

### 验证

- `ait18.py` 基线：26/26 测试通过。
- `ait19.py` 修改后：30/30 测试通过；新增滚动一分钟、平均速度、ETA、初始错误基线、乱序快照和结束冻结测试。
- Tkinter GUI 控件构建与实时文本更新完成无窗口 smoke test。

## v0.18.0-step23-persistence-pipeline — 2026-08-12

### 20 线程持久化流水线

- 正式源码新增 `ait18.py`，`ait17.py` 保持不改。
- 新增 `ResultPersistenceWriter` 单独线程，浏览器 worker 在累计 CSV 实时 `flush/fsync` 后立即继续页面清理和下一条，不再同步等待输入文件整表重写。
- 累计输出第一列只在批次启动时读取一次；每次成功追加后更新内存去重集合。
- 输入第一列删除按 20 个结果或 5 秒批量执行，停止、正常结束和持久化线程关闭时强制刷新。
- 输出已落盘但输入文件暂时被占用时保留待删除键；下次启动继续按累计输出第一列同步，不重复提交。

### 验证与效率

- `ait17.py` 基线：21/21 测试通过。
- `ait18.py`：26/26 测试通过，新增批量合并、浏览器不等待慢删行、定时刷新、重复去重和写入失败恢复测试。
- 当前 4,945 行测试 XLSX 隔离副本的 20 条结果基准：旧版 18.883 秒，新版 1.527 秒，持久化路径提升 12.37 倍；两边均准确删除 20 行，新版准确输出 20 行。
- GUI 上次配置已保存为 `playwright + 窗口 + 20线程`。
- 后续 GitHub Release 默认只上传源码 ZIP、更新日志和源码校验文件，不构建或上传 EXE。
