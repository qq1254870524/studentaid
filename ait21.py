...
APP_TITLE = "StudentAid 批量处理工具 - 第二十六步整行去重版"
...
def _canonical_input_columns(
    ...
    """生成不含页面结果列的规范化输入行，供去重、输出和删行共同使用。"""
    original = [_cell_to_text(value) for value in original_fields]
    mapping = _mapping_dict(input_mapping)
    if original and "full_name" in mapping:
        # 新格式保留整条原始行。这样去重键包含每一个源字段，而不是只看
        # SSN、姓名或生日等单列；同一 SSN 但任意其他列不同都会分别处理。
        return [
    ...
    if original and _row_looks_like_compact_date(original):
        return [
            details.original_ssn or details.ssn,
            _format_output_dob(details),
            details.first_name,
...
def read_output_row_keys(output_path: Path) -> set[str]:
    ...
    return {
        json.dumps(
            [_normalise_row_field(value) for value in row[:-4]],
            ensure_ascii=False,
            separators=(",", ":"),
        )
        for row in rows
        if len(row) >= 4 and any(value.strip() for value in row[:-4])
    }
            ...
            try:
                for worksheet in workbook.worksheets:
                    ...
                    for row_number in range(first_data_row, worksheet.max_row + 1):
                        values = [
                            _cell_to_text(worksheet.cell(row_number, column).value)
...
def remove_duplicate_input_rows(input_path: Path) -> int:
    ...
    with _SOURCE_FILE_LOCK:
        ...
        if suffix == ".xlsx":
            ...
            try:
                for worksheet in workbook.worksheets:
                    header = [_cell_to_text(cell.value) for cell in worksheet[1]]
...
class ResultPersistenceWriter:
    ...
    浏览器 worker 只同步等待累计 CSV flush/fsync。完整输入行键在 CSV 安全落盘后
    进入待删除集合，由本线程按数量或时间批量修改；异常退出时启动同步仍会按完整行
    清理输入，因此不会重复提交已经有明确输出的完全相同资料。
    """
...
class BatchEngine:
    ...
    def _run(
        ...
        total = 0
        try:
            self._emit("log", message="正在读取累计输出完整资料行并同步输入文件……")
