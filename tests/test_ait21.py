...
if not (module_root / "ait21.py").is_file():
    module_root = module_root.parent
sys.path.insert(0, str(module_root))

import ait21 as ait11
...
class Step23PersistencePipelineTests(unittest.TestCase):
    def test_same_ssn_different_rows_are_both_appended_and_deleted(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path(__file__).parent) as directory:
            ...
            self.assertEqual("ADDRESS", rows[0][4])
            self.assertEqual("ADDRESS 2", rows[1][4])
...
class Step25MyLifeFormatAdaptationTests(unittest.TestCase):
    ...
    def test_output_starts_with_ssn_and_preserves_unmapped_columns(self) -> None:
        ...
        # 4 个规范身份字段 + 全部 20 个原始字段，再追加 4 个结果列。
        # 去重因此覆盖整条原始行，不能退化成只按 SSN 等单列去重。
        self.assertEqual(28, len(output))
        self.assertIn("2817334560", output[4:-4])
        self.assertIn("487-78-6185", output[4:-4])
        self.assertIn("1972-09-06", output[4:-4])
        self.assertIn("Mary Dangerfield", output[4:-4])
