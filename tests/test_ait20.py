...
if not (module_root / "ait20.py").is_file():
    module_root = module_root.parent
sys.path.insert(0, str(module_root))

import ait20 as ait11
...
class Step25MyLifeFormatAdaptationTests(unittest.TestCase):
    ...
    def test_output_starts_with_ssn_and_preserves_unmapped_columns(self) -> None:
        mapping = ait11._header_mapping(self.header)
        assert mapping is not None
        ...
        self.assertEqual(
            ["487-78-6185", "09/07/1972", "Mary", "Dangerfield"],
            output[:4],
        )
        self.assertEqual("487786185", item.record_key)
        # 4 个身份逻辑字段占 4 个源列，余下 16 个原始列完整顺延，再追加 4 个结果列。
        self.assertEqual(24, len(output))
        self.assertIn("2817334560", output[4:-4])
        self.assertIn("Tomball", output[4:-4])
        self.assertIn("matched", output[4:-4])
    ...
    def test_delete_by_output_ssn_uses_mapped_second_column(self) -> None:
        with tempfile.TemporaryDirectory(dir=Path(__file__).parent) as directory:
            ...
            with path.open("w", encoding="utf-8-sig", newline="") as stream:
                csv.writer(stream).writerows(rows)
            removed = ait11.remove_input_rows_by_keys(path, {"487786185"})
    ...
    def test_uploaded_file_imports_94_valid_and_2_missing_dob(self) -> None:
        path = Path(r"E:\studentaid\需要查生日_MyLife结果.csv")
        if not path.is_file():
            self.skipTest("现场 MyLife 新格式文件不存在")
        ...
        self.assertEqual(96, len(records))
        self.assertEqual(94, len(valid))
        self.assertEqual(2, len(missing))
        self.assertTrue(all(record.details is not None for record in valid))
