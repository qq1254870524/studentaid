...
APP_TITLE = "StudentAid 批量处理工具 - 第二十五步 MyLife 新格式自适应版"
...
def load_gui_config(config_path: Path | None = None) -> dict[str, str]:
    ...
    if isinstance(backend, str) and backend in BROWSER_BACKENDS:
...
class WorkItem:
    ...
    def record_key(self) -> str:
        return _normalise_record_key(self.details.ssn)
...
HEADER_ALIASES: Mapping[str, set[str]] = {
    ...
    "dob": {"dob", "dateofbirth", "birthdate", "knownbirthday"},
...
def _header_mapping(row: Sequence[str]) -> dict[str, int] | None:
    ...
    if "ssn" in mapping and has_date and has_name:
        return mapping
    return None
    ...
    if header is not None:
        ...
        if "full_name" in header:
            first_name, last_name = _split_full_name(
                _mapped_value(row, header, "full_name")
            )
        else:
        ...
        dob = (
            _mapped_value(row, header, "preferred_dob")
            or _mapped_value(row, header, "dob")
        )
        if dob:
            month, day, year = _parse_dob(dob)
        elif "preferred_dob" in header or "dob" in header:
            month = day = year = ""
        else:
...
def remove_input_rows_by_keys(input_path: Path, keys: set[str]) -> int:
    """按逻辑 SSN 列原子删除输入行；XLSX 保留样式和其他工作表。"""
    ...
    with _SOURCE_FILE_LOCK:
        ...
        if suffix in {".csv", ".scv", ".txt"}:
            ...
            key_index = mapping.get("ssn", 0) if mapping else 0
            for index, row in enumerate(rows):
                if index == 0 and mapping is not None:
                    kept.append(row)
                    continue
                key = (
                    _normalise_record_key(row[key_index])
        ...
        if suffix == ".xlsx":
            ...
            try:
                for worksheet in workbook.worksheets:
                    ...
                    key_column = mapping.get("ssn", 0) + 1 if mapping else 1
                    rows_to_delete = []
                    for row_number in range(first_data_row, worksheet.max_row + 1):
                        key = _normalise_record_key(
                            worksheet.cell(row_number, key_column).value
                        )
                        if key and key in keys:
...
def build_cumulative_output_row(item: WorkItem, result: RecoveryResult) -> list[str]:
    """透传紧凑输入的动态列，并在末尾追加四个页面结果字段。"""
    first_column = item.details.original_ssn or item.details.ssn
    original_fields = list(item.original_fields)
    input_mapping = _mapping_dict(item.input_mapping)
    if original_fields and "full_name" in input_mapping:
        # 已识别的 MyLife 表头格式：逻辑 SSN/DOB/First/Last 放到前四列，
        # primary_phone 等未参与身份映射的列继续按原顺序透传。
        consumed_indices = {
            index
            for key, index in input_mapping.items()
            if key in {"ssn", "dob", "preferred_dob", "full_name"}
        }
        input_columns = [
            first_column,
            ...
            *(
                value
                for index, value in enumerate(original_fields)
                if index not in consumed_indices
            ),
        ]
    elif original_fields and _row_looks_like_compact_date(original_fields):
        # 前四个逻辑字段固定；第 5 列起保持原列边界和顺序。
        input_columns = [
            first_column,
            _format_output_dob(item.details),
            item.details.first_name,
...
def append_cumulative_result_cached(
    ...
    """使用批次内存去重集合实时追加，不为每条结果重新扫描累计 CSV。"""
    output_path = output_path.resolve()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with _SOURCE_FILE_LOCK:
        ...
        with output_path.open("a", encoding="utf-8-sig", newline="") as stream:
            csv.writer(
                stream, lineterminator="\n", quoting=csv.QUOTE_ALL
            ).writerow(build_cumulative_output_row(item, result))
            stream.flush()
            os.fsync(stream.fileno())
        if record_key:
...
class ResultPersistenceWriter:
    """串行写累计 CSV，并在后台合并输入文件删行。

    浏览器 worker 只同步等待累计 CSV flush/fsync。输入第一列在 CSV 安全落盘后进入
    待删除集合，由本线程按数量或时间批量修改；异常退出时启动同步仍会按输出第一列
    清理输入，因此不会重复提交已经有明确输出的资料。
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.existing_keys = {
            key for value in existing_keys if (key := _normalise_record_key(value))
        }
        ...
        self._pending_delete_keys: set[str] = set()
    ...
    def _run(self) -> None:
        ...
        while not closing:
            ...
            if pending_count >= self.batch_size or (pending_count and flush_due):
